...
const cryptoToSell: CryptoToSell = reactive({
  cryptoSymbol: '',
  cryptoCurrentValue: 0,
  valueToSell: 0,
  valueInCrypto: computed(() => (cryptoToSell.valueToSell / cryptoToSell.cryptoCurrentValue) || 0),
})
  ...
  <form class="form-sell-order-pix" @submit.prevent="sellOrderStore">
    ...
    <fieldset class="group">
      ...
      <!-- <Textfield
        ...
        class="textfield"
        inputmode="numeric"
      placeholder="Valor na cripto"-->
      <!-- /> -->

      <strong class="font-display text-xs ">{{ cryptoToSell.cryptoSymbol }} {{ cryptoToSell.valueInCrypto }}</strong>
    </fieldset>
